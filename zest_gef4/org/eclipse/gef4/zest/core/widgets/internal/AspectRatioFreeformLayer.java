...
public class AspectRatioFreeformLayer extends FreeformLayer implements
	...
	protected void paintClientArea(Graphics graphics) {
		...
		if (!optimizeClip) {
			g.clipRect(shrink(getBounds().getCopy (), getInsets()));
		}
	...
	public static Rectangle shrink(Rectangle source, Insets insets) {
		if (insets == null)
			return source;
		source.x += insets.left;
		source.y += insets.top;
		source.width -= (insets.getWidth());
		source.height -= (insets.getHeight());
		return source;
	}
}
