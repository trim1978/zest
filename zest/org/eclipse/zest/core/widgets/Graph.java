...
public class Graph extends FigureCanvas implements IContainer2 {
	...
	public String toString() {
		return "GraphModel {"+nodes.size()+" nodes, "+connections.size()+" connections}"; //$NON-NLS-1$
	}
		...
		public void mouseDoubleClicked(org.eclipse.draw2d.MouseEvent me) {
			...
			if (itemUnderMouse instanceof GraphContainer) {
				...
				while (currentContainer instanceof GraphContainer) {
					GraphContainer current = (GraphContainer)currentContainer;
				...
				for (GraphNode node : container.getNodes()) {
					...
					if (node instanceof GraphContainer) {
						g.registerChildrenOfContainer((GraphContainer)node, true); // recursively add childNodes to graph
					}
				}
				...
				backButton.addActionListener(event -> {
					for (GraphNode node : new ArrayList<>(g.getNodes())) {
						...
						if (node instanceof GraphContainer) {
							registerChildrenOfContainer((GraphContainer)node, false); // recursively add childNodes to graph
						}
					}
	...
	private void registerChildrenOfContainer(GraphContainer container, boolean addToMap) {
		for (GraphNode node : container.getNodes()) {
			if (node instanceof GraphContainer) {
				registerChildrenOfContainer((GraphContainer)node, addToMap);
			} else {
				node.graph = this;
				node.unhighlight();
				if (addToMap) {
		...
		synchronized (this) {
			if (scheduledLayoutRunnable == null) {
				Display.getDefault().asyncExec(scheduledLayoutRunnable = () -> {
					...
					if (getLayoutAlgorithm() instanceof LayoutAlgorithm.Zest1) {
						LayoutAlgorithm.Zest1 zest1 = (LayoutAlgorithm.Zest1)getLayoutAlgorithm();
						try {
