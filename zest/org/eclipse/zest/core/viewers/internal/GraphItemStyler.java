...
public class GraphItemStyler {
	...
	private static void styleConnection(GraphConnection conn, IConnectionStyleProvider provider) {
		...
		if (provider instanceof IConnectionStyleProvider2) {
			ConnectionRouter cr;
			if ((cr = ((IConnectionStyleProvider2)provider).getRouter(rel)) != null) {
				conn.setRouter(cr);
			}
		}
	}
	...
	private static void styleEntityConnection(GraphConnection conn, IEntityConnectionStyleProvider provider) {
		...
		if (provider instanceof IEntityConnectionStyleProvider2) {
			IFigure tooltip;
			if ((tooltip = ((IEntityConnectionStyleProvider2)provider).getTooltip(src, dest)) != null) {
				conn.setTooltip(tooltip);
			}
		}
		int w = -1;
		...
		if (provider instanceof IEntityConnectionStyleProvider2) {
			ConnectionRouter cr;
			if ((cr = ((IEntityConnectionStyleProvider2)provider).getRouter(src, dest)) != null) {
				conn.setRouter(cr);
			}
		}
	}
