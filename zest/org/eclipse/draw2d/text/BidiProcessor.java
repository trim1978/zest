...
public final class BidiProcessor {
	...
	private static final BidiProvider BIDI = FlowUtilities.findFirst(ServiceLoader.load(BidiProvider.class)).orElseGet(DefaultBidiProvider::new);
	...
	/**
	 * Processes the contributed text, determines the Bidi levels, and assigns them
	 * to the FlowFigures that made thet contributions. This class is for INTERNAL
	 * use only. Shaping of visually contiguous Arabic characters that are split in
	 * different figures is also handled. This method will do nothing if the
	 * contributed text does not require Bidi evaluation. All contributions are
	 * discarded at the end of this method.
	 */
	public void process() {
		try {
