...
public class Label extends Figure implements PositionConstants {
	...
	private void alignOnHeight(Point loc, Dimension size, int alignment) {
		...
		loc.y = alignment == TOP ? insets.top : alignment == BOTTOM ? bounds.height - size.height - insets.bottom : (bounds.height - size.height) / 2;
	}

	private void alignOnWidth(Point loc, Dimension size, int alignment) {
		Insets insets = getInsets();
		loc.x = alignment == LEFT ? insets.left : alignment == RIGHT ? bounds.width - size.width - insets.right : (bounds.width - size.width) / 2;
	}

	private void calculateAlignment() {
		if (textPlacement == EAST || textPlacement == WEST) {
			alignOnHeight(textLocation, getTextSize(), textAlignment);
			alignOnHeight(iconLocation, getIconSize(), iconAlignment);
		}
		if (textPlacement == NORTH || textPlacement == SOUTH) {
	...
	protected Dimension calculateLabelSize(Dimension txtSize) {
		...
		} else if (textPlacement == SOUTH || textPlacement == NORTH) {
